...
function CustomerProfileContent() {
  ...
  const handleOpenEditOrderAddress = (ord: any) => {
    setEditingOrder(ord);
    setOrderAddressForm({
      customerName: ord.customerName || profile?.fullName || '',
      customerPhone: ord.customerPhone || profile?.phone || '',
      province: ord.province || 'Hà Nội',
      district: ord.district || '',
      ward: ord.ward || '',
      addressDetail: ord.addressDetail || '',
      updateDefaultAddress: false,
    });
    setShowOrderAddressModal(true);
  };
  ...
  const handleSaveOrderAddress = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowOrderAddressModal(false);
    } catch (err: any) {
      alert(err?.message || 'Không thể cập nhật địa chỉ đơn hàng.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F9F6F0] py-10 px-4 sm:px-6 lg:px-8">
                      ...
                      <div className="text-xs text-slate-500 space-y-0.5">
                        ...
                        {(ord.status === 'PENDING' || ord.status === 'PROCESSING') && (
                          <button
                            ...
                            className="mt-1.5 inline-flex items-center gap-1.5 text-xs font-semibold text-[#2D5A27] bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 px-2.5 py-1 rounded-lg transition-colors"
                          >
                            <MapPin className="w-3.5 h-3.5" /> Sửa địa chỉ nhận hàng
                          </button>
                        )}
                        {ord.pointsUsed > 0 && (
          ...
          <div className="bg-white max-w-lg w-full rounded-2xl shadow-xl p-6 space-y-4">
            ...
            <form onSubmit={handleSaveAddress} className="space-y-4 text-sm">
              ...
              <AdministrativeSelects
                ...
                onChange={(field, val) => setAddressForm((prev) => ({ ...prev, [field]: val }))}
              />
      ...
      {showOrderAddressModal && editingOrder && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white max-w-lg w-full rounded-2xl shadow-xl p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-lg font-bold text-[#1E293B]">
                Sửa Địa Chỉ Giao Hàng (Đơn: #{editingOrder.orderNumber || editingOrder.id})
              </h3>
            ...
            <form onSubmit={handleSaveOrderAddress} className="space-y-4 text-sm">
              <div>
                <label className="block font-medium text-slate-700 mb-1">Tên người nhận</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block font-medium text-slate-700 mb-1">Số điện thoại nhận hàng</label>
                <input
                  type="tel"
                  required
              ...
              <AdministrativeSelects
                provinceValue={orderAddressForm.province}
                wardValue={orderAddressForm.ward}
                onChange={(field, val) => setOrderAddressForm((prev) => ({ ...prev, [field]: val }))}
              />
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  value={orderAddressForm.addressDetail}
                  onChange={(e) => setOrderAddressForm({ ...orderAddressForm, addressDetail: e.target.value })}
                  className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2D5A27]"
                />
              </div>
              ...
              <label className="flex items-center gap-2 text-slate-700 cursor-pointer pt-1">
                <input
                  type="checkbox"
                  checked={orderAddressForm.updateDefaultAddress}
                  onChange={(e) => setOrderAddressForm({ ...orderAddressForm, updateDefaultAddress: e.target.checked })}
                  className="w-4 h-4 rounded text-[#2D5A27] focus:ring-[#2D5A27]"
                />
                <span>🔥 Đồng thời cập nhật địa chỉ này vào Sổ địa chỉ mặc định của tôi</span>
              </label>

              <div className="pt-4 flex items-center justify-end gap-3">
                <button
                  type="button"
                  onClick={() => setShowOrderAddressModal(false)}
                  className="px-4 py-2 border border-slate-200 rounded-xl text-slate-600 hover:bg-slate-50"
                >
                  Hủy
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-[#2D5A27] text-white font-medium rounded-xl hover:bg-[#23471E] disabled:opacity-50"
                >
                  {updateOrderAddressMutation.isPending ? 'Đang cập nhật...' : 'Cập Nhật Địa Chỉ'}
                </button>
              </div>
            </form>
          </div>
